...
class PathsManager:
    ...
    def _load_paths(self) -> None:
        ...
        if os.path.exists(env_paths_file):
            try:
                with open(env_paths_file, 'r') as f:
                    self.paths = json.load(f)
                self.logger.info(f"Loaded paths from {env_paths_file}")
                
                # Ensure all paths exist
                for name, path in self.paths.items():
                    if not os.path.exists(path):
                        self.logger.warning(f"Path {name} = '{path}' does not exist")
                        
            except Exception as e:
                self.logger.error(f"Error loading paths from {env_paths_file}: {e}")
                # Initialize with empty paths
                self.paths = {}
        else:
            self.logger.warning(f"Paths file not found: {env_paths_file}")
            # Initialize with empty paths
            self.paths = {}
    ...
    def save_paths(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Saved paths to {env_paths_file}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error saving paths: {e}")
            return False
