...
class IntegrationInterface:
    ...
    def __init__(self, config_path: str = None):
        ...
        """
        self.nodes = {}  # Словарь для хранения информации об узлах
        self.links = {}  # Словарь для хранения информации о связях между узлами
        self.transactions = []  # Список транзакций
        self.simulation_time = 0.0  # Текущее время симуляции
        self.config = {}  # Конфигурация симуляции
    ...
    def register_node(self, node_id: str, 
        ...
        logger.info(f"Узел {node_id} успешно зарегистрирован")
        return True
    ...
    def update_node_position(self, node_id: str, 
        ...
        logger.debug(f"Узел {node_id} переместился из {old_position} в {position}")
        return True
    
    def register_connection(self, node1_id: str, node2_id: str, 
                          quality: float, bandwidth: float) -> bool:
        """
        ...
        if node1_id not in self.nodes or node2_id not in self.nodes:
            logger.error(f"Один или оба узла не найдены: {node1_id}, {node2_id}")
            return False
        ...
        if connection_id in self.links:
            ...
            logger.debug(f"Соединение {connection_id} обновлено")
        else:
            ...
            logger.info(f"Соединение между узлами {node1_id} и {node2_id} установлено")
        
        return True
    
    def send_transaction(self, source_id: str, target_id: str, 
        ...
        if not self._check_path_exists(source_id, target_id):
            logger.error(f"Нет доступного пути между узлами {source_id} и {target_id}")
            return ""
        ...
        logger.info(f"Транзакция {tx_id} создана и отправлена")
    ...
    def advance_time(self, time_delta: float) -> None:
        ...
        logger.debug(f"Время симуляции продвинуто до {self.simulation_time}")
        ...
        try:
            ...
            if directory and not os.path.exists(directory):
                try:
                    ...
                    logger.error(f"Ошибка прав доступа: невозможно создать директорию {directory}")
                    logger.error("Пожалуйста, проверьте права доступа к файловой системе")
                    return False
            ...
            if os.path.exists(filepath):
                if not os.access(filepath, os.W_OK):
                    logger.error(f"Ошибка прав доступа: отсутствуют права на запись файла {filepath}")
            ...
            logger.info(f"Состояние симуляции сохранено в {filepath}")
            return True
        except PermissionError:
            logger.error(f"Ошибка прав доступа: невозможно записать в файл {filepath}")
    ...
    def load_state(self, filepath: str) -> bool:
        ...
        try:
            ...
            if not os.path.exists(filepath):
                logger.error(f"Файл состояния {filepath} не существует")
                return False
            
            # Проверяем права на чтение
            if not os.access(filepath, os.R_OK):
                logger.error(f"Ошибка прав доступа: отсутствуют права на чтение файла {filepath}")
                logger.error("Пожалуйста, проверьте права доступа к файлу")
                return False
            ...
            logger.info(f"Состояние симуляции загружено из {filepath}")
            return True
        except PermissionError:
            logger.error(f"Ошибка прав доступа: невозможно прочитать файл {filepath}")
            logger.error("Пожалуйста, проверьте права доступа к файлу")
            return False
...
if __name__ == "__main__":
    ...
    tx_id = interface.send_transaction(
        "node_1", 
        "node_2", 
        {"type": "data_transfer", "size": 1024, "priority": "high"}
    )
    ...
    # Сохраняем состояние
    interface.save_state("/tmp/simulation_state.json")
