...
class Blockchain:
    ...
    def __init__(self, difficulty: int = 4):
        ...
        self.nodes: Dict[str, Any] = {}  # node_id -> node_info
    ...
    def mine_pending_transactions(self, miner_reward_address: str) -> Optional[Block]:
        ...
        reward_transaction = {
            ...
            "data": {
                "type": "reward",
                "amount": 1,
            },  # Simple reward of 1 token / Простое вознаграждение в 1 токен
        }
        ...
        new_block = Block(
            latest_block.index + 1, time.time(), transactions_to_mine, latest_block.hash
        )
        ...
        logger.info(
            f"Block #{new_block.index} with {len(transactions_to_mine)} transactions added to the chain"
        )
        return new_block
    ...
    def load_from_file(cls, filepath: str) -> "Blockchain":
        ...
        logger.info(
            f"Blockchain loaded from {filepath} with {len(blockchain.chain)} blocks"
        )
        return blockchain
...
if __name__ == "__main__":
    ...
    blockchain.add_transaction(
        "node_1", "node_2", {"message": "Hello from node 1!", "type": "data"}
    )
    blockchain.add_transaction(
        "node_3", "node_1", {"message": "Response from node 3", "type": "ack"}
    )
    ...
    blockchain.add_transaction(
        "node_2", "node_3", {"message": "Forwarded data", "type": "data"}
    )
