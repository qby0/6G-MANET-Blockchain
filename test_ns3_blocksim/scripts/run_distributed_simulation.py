...
def setup_logging(config):
    ...
    log_file = os.path.join(log_dir, f"distributed_simulation_{datetime.now().strftime('%Y%m%d%H%M%S')}.log")
...
def create_nodes(config, blockchain_manager):
    ...
    logger.info(f"Creating {validator_count} validators and {regular_count} regular nodes")
...
def update_node_positions(config, blockchain_manager, nodes, time_step, ns3_adapter=None):
    ...
    for node_id in nodes:
        if random.random() < 0.8:  # 80% chance of movement in each step
            ...
            if node_id in blockchain_manager.nodes:
                ...
                # Simple random walk
                dx = random.uniform(-1, 1) * speed * time_step
                dy = random.uniform(-1, 1) * speed * time_step
...
def generate_transactions(config, blockchain_manager, nodes, current_time):
    ...
    for node_id in nodes:
        ...
        generation_rate = config["node_properties"][node.node_type]["transaction_generation_rate"]
        ...
        if random.random() < generation_rate:
            ...
            if transaction:
                ...
                logging.debug(f"Transaction from {node_id} propagated to {len(received_nodes)} nodes")
...
def create_blocks(config, blockchain_manager, nodes, current_time):
    ...
    validator_nodes = [
        ...
        if node_id in blockchain_manager.nodes and 
        blockchain_manager.nodes[node_id].node_type == "validator"
    ]
    ...
    if current_time % block_time < 1.0:
        ...
        if validator_nodes:
            ...
            if new_block:
                ...
                # Propagate the block
                received_nodes = blockchain_manager.propagate_block(selected_validator, new_block)
...
def log_metrics(config, blockchain_manager, metrics, current_time):
    ...
    if "connectivity" in config["logging"]["metrics"]:
        ...
        if blockchain_manager.nodes:
            neighbor_counts = [len(node.neighbors) for node in blockchain_manager.nodes.values()]
...
def run_simulation(config, ns3_path=None):
    """Run the distributed blockchain simulation"""
    logger = setup_logging(config)
    logger.info(f"Starting distributed blockchain simulation: {config['simulation_name']}")
    ...
    if ns3_path:
        try:
            logger.info(f"Initializing NS3Adapter with path: {ns3_path}")
            ns3_adapter = NS3Adapter(config_file=os.path.join(get_config_dir(), "distributed.json"), ns3_path=ns3_path)
            logger.info("NS3Adapter initialized successfully")
        except Exception as e:
            logger.error(f"Error initializing NS3Adapter: {e}")
            logger.warning("Continuing simulation without NS-3 integration")
    
    # Create blockchain manager
    config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'config', 'distributed.json'))
    ...
    logger.info(f"Running simulation for {simulation_time} seconds")
    ...
    while current_time < simulation_time:
        ...
        new_transactions = generate_transactions(config, blockchain_manager, nodes, current_time)
        ...
        if int(current_time) % 10 == 0 and current_time > 0:
            ...
            remaining = (elapsed / current_time) * (simulation_time - current_time) if current_time > 0 else 0
            
            logger.info(f"Simulation progress: {progress:.1f}% (Time: {current_time:.1f}s, "
    ...
    status = blockchain_manager.get_network_status()
    logger.info(f"Final network status: {status}")
...
def main():
    parser = argparse.ArgumentParser(description="Run distributed blockchain simulation")
    
    # Change default path to be relative to the script location
    default_config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'config', 'distributed.json'))
    parser.add_argument("-c", "--config", default=default_config_path,
                      help="Path to config file")
    # Add NS-3 path argument
    parser.add_argument("--ns3-path", help="Path to NS-3 installation (optional for NS-3 integration)")
