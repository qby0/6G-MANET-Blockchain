...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("ns3_test.log")],
)
...
def create_test_scenario():
    ...
    links = {
        ...
        "n1_n2": {"nodes": ["node_1", "node_2"], "quality": 0.7, "bandwidth": 50.0},
    }
...
def main():
    ...
    if not args.output_dir:
        ...
        output_dir = os.path.join(
            Path(__file__).parent.parent, "results", f"test_ns3_{timestamp}"
        )
    ...
    try:
        ...
        if not os.path.exists(os.path.join(ns3_path, "ns3")):
            logger.error(
                "NS-3 executable not found. Make sure NS-3 is installed correctly."
            )
            sys.exit(1)
        ...
        logger.info(
            ...
            {"sim_time": results.get("simulation_time", "N/A")},
        )
        logger.info(
            f"- Packets sent: {results.get('network_stats', {}).get('packets_sent', 'N/A')}"
        )
        logger.info(
            f"- Packets received: {results.get('network_stats', {}).get('packets_received', 'N/A')}"
        )
        logger.info(
            f"- Average delay: {results.get('network_stats', {}).get('average_delay', 'N/A')}"
        )
