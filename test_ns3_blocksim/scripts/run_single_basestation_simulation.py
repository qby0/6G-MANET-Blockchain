...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Single Base Station Simulation with AODV Routing')
    
    parser.add_argument('--config', type=str, default='../config/basestation.json',
    ...
    parser.add_argument('--coverage-radius', type=float, default=200.0,
    ...
    parser.add_argument('--movement-speed', type=float, default=5.0,
                      help='Node movement speed in m/s')
...
def is_in_coverage(position: Tuple[float, float, float], 
    ...
    """
    distance = np.sqrt((position[0] - base_station_pos[0])**2 + 
...
def create_network_topology(interface, config, args):
    ...
    base_station_pos = (area_size/2, area_size/2, 20.0)  # Higher elevation for the base station
    ...
    interface.register_node(
        ...
        base_station_pos,
        "base_station",
        {"computational_power": 100, "storage": 1000, "battery": None, "coverage_radius": coverage_radius}
    )
    ...
    for i in range(node_count):
        ...
        is_validator = random.random() < 0.1
    ...
    logger.info(f"Created network topology with 1 base station and {node_count} nodes")
    logger.info(f"Initial nodes in coverage: {nodes_in_coverage}")
    logger.info(f"Initial nodes outside coverage: {nodes_outside_coverage}")
...
def update_connections(interface, base_station_pos, coverage_radius, max_node_range=100.0):
    ...
    for node_id, node_info in interface.nodes.items():
        ...
        if is_in_coverage(position, base_station_pos, coverage_radius):
            ...
            quality = max(0.5, 1.0 - np.sqrt((position[0] - base_station_pos[0])**2 + 
                                           (position[1] - base_station_pos[1])**2) / coverage_radius)
    ...
    logger.debug(f"Total connections in network: {len(interface.links)}")
...
def run_simulation(args, config):
    ...
    while current_time < total_time:
        ...
        for node_id, position in list(node_positions.items()):
            ...
            was_in_coverage = is_in_coverage(position, base_station_pos, args.coverage_radius)
            is_in_coverage_now = is_in_coverage((new_x, new_y, position[2]), base_station_pos, args.coverage_radius)
            
            if was_in_coverage != is_in_coverage_now:
                status = "entered coverage" if is_in_coverage_now else "left coverage"
                logger.debug(f"Node {node_id} {status} at time {current_time:.2f}")
        ...
        if int(current_time * 10) % 100 == 0:  # Every 10 seconds
            ...
            nodes_communicating = min(3, len(interface.nodes) - 1)  # Max 3 nodes communicate at once
            
            # Prefer nodes outside coverage for interesting routing paths
            outside_nodes = [node_id for node_id, pos in node_positions.items()
                            if node_id != "base_station_1" and not 
                            is_in_coverage(pos, base_station_pos, args.coverage_radius)]
            
            # If we have nodes outside coverage, use them
            communicating_nodes = []
            if outside_nodes:
                # Include at least one node outside coverage if available
                communicating_nodes.append(random.choice(outside_nodes))
                nodes_communicating -= 1
            
            # Add random nodes for remaining slots
            available_nodes = [n for n in interface.nodes.keys() 
                              if n != "base_station_1" and n not in communicating_nodes]
            
            communicating_nodes.extend(random.sample(available_nodes, 
                                                   min(nodes_communicating, len(available_nodes))))
            ...
            for node_id in communicating_nodes:
                ...
                logger.debug(f"Transaction from {node_id} to base_station_1 - Direct path: {direct_path}")
                ...
                if tx_id:
                    blocksim_adapter.create_transaction(node_id, "base_station_1", tx_data)
            ...
            if processed > 0:
                ...
                if block:
                    logger.info(f"New block {block['index']} created by base station")
        ...
        if int(current_time * 10) % 200 == 0 or current_time >= total_time - time_step:
            ...
            logger.info(f"Nodes in coverage: {nodes_in_coverage}, outside: {nodes_outside_coverage}")
            logger.info(f"Total connections: {len(net_state['links'])}")
            ...
            if args.save_interim and int(current_time * 10) % 500 == 0:
                ...
                # Save coverage data
                with open(os.path.join(interim_output, "coverage_changes.json"), 'w') as f:
                    json.dump(coverage_changes, f, indent=2)
                
                # Save routing data
                with open(os.path.join(interim_output, "transaction_routes.json"), 'w') as f:
    ...
    with open(os.path.join(output_dir, f"coverage_changes_{timestamp}.json"), 'w') as f:
        json.dump(coverage_changes, f, indent=2)
    
    with open(os.path.join(output_dir, f"transaction_routes_{timestamp}.json"), 'w') as f:
        json.dump(transaction_hops, f, indent=2)
    ...
    summary = {
        ...
        "coverage_changes_file": os.path.join(output_dir, f"coverage_changes_{timestamp}.json"),
        "transaction_routes_file": os.path.join(output_dir, f"transaction_routes_{timestamp}.json"),
