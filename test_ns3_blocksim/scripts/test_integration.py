...
from models.paths_manager import get_ns3_dir, get_config_dir, get_all_paths
...
def main():
    ...
    for name, path in all_paths.items():
        ...
        if not os.path.exists(path):
            logger.warning(f"WARNING: Path {name} does not exist: {path}")
    
    # Test NS-3 path
    ns3_path = get_ns3_dir()
    if ns3_path:
        logger.info(f"NS-3 path found: {ns3_path}")
        
        # Check if NS-3 path exists
        if os.path.exists(ns3_path):
            logger.info(f"NS-3 directory exists at: {ns3_path}")
            ...
            try:
                ...
                logger.info(f"NS-3 version: {version}")
                ...
                if ns3_configured:
                    ...
                    logger.warning("NS-3 may not be properly configured. Could not find 'ns3' script.")
            ...
            logger.error(f"NS-3 directory does not exist at: {ns3_path}")
        ...
        logger.info("Please set NS3_DIR in env_paths.json or as an environment variable")
        return 1
    ...
    if config_dir:
        ...
        if os.path.exists(integrated_config):
            logger.info(f"Integrated config file found: {integrated_config}")
            
            # Read the config file
            try:
                with open(integrated_config, 'r') as f:
                    config = json.load(f)
                logger.info(f"Successfully loaded integrated.json with {len(config)} keys")
                
                # Check essential configuration
                if "simulation_name" in config:
                    logger.info(f"Simulation name: {config['simulation_name']}")
                else:
                    logger.warning("Missing 'simulation_name' in config")
                
                if "simulation_time" in config:
                    logger.info(f"Simulation time: {config['simulation_time']} seconds")
                else:
                    logger.warning("Missing 'simulation_time' in config")
                
                if "network" in config:
                    logger.info(f"Network config: {len(config['network'])} parameters")
                else:
                    logger.warning("Missing 'network' section in config")
                
                if "blockchain" in config:
                    logger.info(f"Blockchain config: {len(config['blockchain'])} parameters")
                else:
                    logger.warning("Missing 'blockchain' section in config")
                
            except Exception as e:
                logger.error(f"Error reading integrated.json: {e}")
        else:
            logger.warning(f"Integrated config file not found: {integrated_config}")
    else:
