...
from models.paths_manager import get_ns3_dir, get_config_dir, get_path, get_results_dir
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(log_dir, f"integrated_sim_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")),
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Run integrated blockchain simulation with NS-3")
    
    # NS3 path can be set in paths_manager, environment, or command line
    parser.add_argument("--ns3-path", help="Path to NS-3 installation (if not set in paths_manager or NS3_DIR environment variable)")
    
    parser.add_argument("--config", help="Path to configuration file")
    parser.add_argument("--output-dir", help="Output directory for simulation results")
    parser.add_argument("--simulation-time", type=float, help="Simulation time in seconds")
    parser.add_argument("--node-count", type=int, help="Number of nodes in the simulation")
    parser.add_argument("--validator-percentage", type=float, help="Percentage of nodes that are validators")
    parser.add_argument("--animation", action="store_true", help="Generate NS-3 animation")
...
def get_ns3_path(args):
    ...
    if args.ns3_path and os.path.exists(args.ns3_path):
        logger.info(f"Using NS-3 path from command line: {args.ns3_path}")
    ...
    if ns3_path and os.path.exists(ns3_path):
        logger.info(f"Using NS-3 path from environment variable: {ns3_path}")
        return ns3_path
    
    logger.error("NS-3 path not found. Please set it in paths_manager, NS3_DIR environment variable, or command line.")
    return None
...
def load_config(args):
    ...
    if args.output_dir:
        ...
        if "save_path" not in config["output"]:
            ...
            config["output"]["save_path"] = os.path.join(get_results_dir(), f"integrated_sim_{timestamp}")
...
def run_simulation(config_path, ns3_path):
    ...
    logger.info(f"NS-3 path: {ns3_path}")
    ...
    try:
        ...
        logger.info(f"Transactions: {results['blockchain']['transactions']}")
        logger.info(f"Blocks: {results['blockchain']['blocks']}")
        
        if 'final_status' in results['blockchain']:
            network_status = results['blockchain']['final_status']
            logger.info(f"Network status: {network_status['total_nodes']} nodes, "
        ...
        summary_file = os.path.join(get_results_dir(), f"summary_{datetime.now().strftime('%Y%m%d%H%M%S')}.json")
...
def main():
    ...
    try:
        ...
        logger.info(f"Simulation completed. Results saved to: {summary_file}")
        return 0
